...
@Component({
  ...
  template: `
    ...
    <ul>
      ...
      <li><strong>Dynamic name provisioning needs a broad grant on Docker - read this.</strong> To create <code>-s</code> names without a redeploy, the agent needs the Docker socket (Docker/Swarm) or a namespace-scoped RBAC role (Kubernetes: manage Services and the endpoints of the Services plug itself creates, plus get/list/patch on Deployments so <code>plug update</code> can roll the agent's own - nothing else, and no access to pods). The Docker socket is <em>not</em> tight - it is root on the host; plug uses it only to create and delete signpost containers (or, when the agent runs as a Swarm service, signpost <em>services</em>) and to refresh its own deployment on <code>plug update</code>, but the capability it hands over is broad, so mount it only on a trusted dev cluster. It is not optional: an agent without it refuses to start, because provisioning names is what it is deployed for. The agent-side helper is itself locked down: it is the tunnel user's <code>ForceCommand</code>, exposing only <code>serve-name</code>/<code>unserve-name</code>/<code>info</code>/<code>resolve</code>/<code>self-update</code> with validated arguments - no shell. Note <code>self-update</code> honestly: anyone who can reach the agent port can trigger a redeploy of the agent - and, since <code>plug update &lt;tag&gt;</code>, can choose <em>which tag</em> of that repository it lands on, an older release included. The repository itself is fixed by the deployment, so this is never an arbitrary image; but the version is caller-chosen, which means a downgrade to a release with known issues is within reach of whoever can reach the port. Same trust boundary as the rest of plug - the agent port is not a public one.</li>
