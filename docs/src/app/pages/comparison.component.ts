    ...
    <div class="cmp">
      <table>
        ...
        <tbody>
          ...
          <tr><td>Inherit the workload's environment</td><td class="win">✓ by default on a takeover, secrets as the pod has them; the <strong>cluster's value wins</strong> over an inherited one (behaves as in the cluster), and <code>--no-env</code> keeps yours instead - whole OR key by key (<code>--no-env DB_URL,API_KEY</code>)</td><td>✓ by default, from the target pod</td><td>✓ on intercept, or <code>--env-file</code></td></tr>
