...
hr "CASE 2 (required) — Go raw-TCP reaches cluster web BY NAME under plug (seccomp supervisor)"
out=$(PLUG_HOOK_DEBUG=1 plug -H "$AG" --port "$PORT" goraw web:8080 2>&1)
echo "$out" | sed 's/^/    /'
if echo "$out" | grep -q CLUSTER-OK; then echo "  PASS ✅  (Go is covered!)"; else echo "  FAIL ❌"; fail=1; fi
