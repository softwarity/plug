...
- [ ] **`kill-*` avale l'échec d'une annulation** (17/08) : chaque `gh run cancel` porte un `|| true`, donc un refus laisse le job VERT et six clusters tournent jusqu'à leur TTL sans que rien ne le dise. Vérifié ce jour : les quatre annulations passent (`✓ Request to cancel workflow … submitted`) et les runs s'arrêtent en une à deux minutes — l'asynchronisme est normal, ce n'est pas le défaut. Le défaut est la même forme de mensonge que le `continue-on-error` retiré du selftest Windows : constater l'acceptation et le dire sinon.
